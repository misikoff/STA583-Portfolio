...
I've put together an R package, `TailRiskAnalyzer` available at https://misikoff.github.io/TailRiskAnalyzer/. This package includes many functions for exploring the implications of fat tails. These functions range the analytical to the demonstrative, and help users analyze distributions for "fat-tailedness" and understand the implications of "fat-tailedness".
...
- `create_side_by_side_plot`: plot outcome distributions for different betting strategies, highlighting the difference between median outcome and expected value, and ensemble averages compared to time
averages
...
These relatively simple functions can be helpful for developing better intuitions about distributions and their properties. For example in the second plot of Figure 1, the behavior of the sample mean from a Cauchy distribution is compared to a Normal(0,1) distribution. While the Normal(0, 1) converges quickly, the Cauchy distribution does not converge at all, as the Central Limit Theorem does not apply, given the Cauchy distribution's undefined mean and variance. In fact the sample mean of $\text{Cauchy}(0, 1)$ distribution is also $\text{Cauchy}(0, 1)$. Graphs like this can be a helpful tool for understanding what this property means in practice.
...
- **Out-of-the-Money**: A call option is out-of-the-money if the stock price is below the strike price. A put option is out-of-the-money if the stock price is above the strike price.
...
- rebalance portfolio between SPY and put options
  1. 95.5%: SPY
  2. 0.5%: SPY put options that are 30% "out-of-the-money", with expirations 60 days in the future
